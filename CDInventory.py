...
class FileProcessor:
    """Handling of data in and out of a file"""
    ...
    def update_file(file_name, table):
        """Function that overwrites text file with updated inventory data
        ...
        Returns:
            None
        """
        objFile = open(file_name, 'w') # this opens the text file to overwrite with data in inventory
        for row in table:
...
while True:
    ...
    # 3.5 process delete a CD
        ...
        DataProcessor.del_Inv(intIDDel)
        IO.show_inventory(lstTbl)
        continue  # start loop back at top.
